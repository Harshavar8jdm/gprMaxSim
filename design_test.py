...
class PlotBScanDialog(QDialog):
    def __init__(self):
        ...
        self.gpu_dropdown = QComboBox()
        self.gpu_dropdown.addItems([
            "Yes", "No"
        ])

        self.gpu_dropdown.setCurrentText("No")
        layout.addRow("Gpu:", self.gpu_dropdown)

        self.vel_label = QLabel("Velocity (m/s):")
        self.vel_input = QLineEdit()
        self.vel_input.setPlaceholderText("e.g. 1.22e8")
        layout.addRow(self.vel_label, self.vel_input)  
...
class GPRViewer(QMainWindow):
    ...
    def plot_ascan_file(self):
        out_file, ok = QInputDialog.getText(
            self, "Plot A-scan", "Enter .out filename (e.g., file1.out):"
        )
        ...
        cmd = f"python -m tools.plot_Ascan {out_file}"
        self.shell_output.appendPlainText(f"> {cmd}")
        self.run_command(cmd)
