...
class GPRMaxInputGenerator(QWidget):
    ...
    def initUI(self):
        ...
        self.export_json_btn = QPushButton("Export Project")
        self.import_json_btn = QPushButton("Import Project")
        self.load_in_btn = QPushButton("Load .in File")
        new_btns.addWidget(self.reset_btn)
        new_btns.addWidget(self.save_as_btn)
        new_btns.addWidget(self.export_json_btn)
        new_btns.addWidget(self.import_json_btn)
        new_btns.addWidget(self.load_in_btn)
        ...
        self.export_json_btn.clicked.connect(self.export_project_json)
        self.import_json_btn.clicked.connect(self.import_project_json)
        self.load_in_btn.clicked.connect(self.load_input_file)
    ...
    def load_input_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "Load Input File", "", "Input Files (*.in)")
        if path:
            try:
                with open(path, 'r') as f:
                content = f.read()
                QMessageBox.information(self, "Loaded", f"Input file loaded:
